...
// Mapping slots for furniture items
const FURNITURE_SLOTS: Record<string, "wall" | "corner" | "floor"> = {
  fur_lantern: "wall",
  fur_bonsai: "corner",
  fur_maneki: "corner",
  fur_cushion: "floor",
...
export function useShibaRoom() {
  ...
  const getShibaMascot = () => {
    ...
    if (equippedFloor === "fur_cushion") {
      return {
        gif: "/images/shiba_master.gif",
        style: { bottom: "35%", left: "43%", width: "26%" },
      };
    }
    if (equippedFloor === "fur_kotatsu") {
      return {
        gif: "/images/shiba_master.gif",
  ...
  const isItemEquipped = (item: GachaItem) => {
    if (item.type === "furniture") {
      const slot = FURNITURE_SLOTS[item.id];
      return userStats.equippedFurniture?.[slot] === item.id;
    }
    if (item.type === "voice") {
