    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-20">
      {petals.map((petal) => (
        <motion.div
          ...
          animate={{ y: ["0vh", "110vh"], x: [0, Math.random() * 100 - 50, Math.random() * 100 - 50], rotate: [petal.rotate, petal.rotate + 360], opacity: [0, 1, 1, 0] }}
...
export function EndScreen({ resetProgress }: EndScreenProps) {
  return (
    <div className="flex flex-col items-center justify-center px-4 pt-10 relative w-full min-h-[500px]">
      ...
      <div className="relative z-10 w-full max-w-md mx-auto bg-white/95 backdrop-blur-sm p-8 rounded-[3rem] border-4 border-[#FFD166] shadow-[0_12px_0_0_#FFD166] text-center flex flex-col items-center animate-in zoom-in-90 duration-500">
        ...
        <h2 className="text-4xl text-[#FF9F1C] mb-2 leading-snug" style={{ fontFamily: "var(--font-cherry)", filter: "drop-shadow(0px 3px 0px rgba(255, 209, 102, 1))" }}>
          Giỏi quá ta ơi!
        </h2>
        <p className="font-rounded font-bold text-amber-700 mb-8 text-lg">
          Bạn đã cày nát bộ bài này rồi
        </p>
        <button onClick={resetProgress} className="w-full bg-[#06D6A0] hover:bg-[#05b889] text-white h-14 rounded-2xl font-bold text-xl border-b-4 border-[#048c68] active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center shadow-sm">
