    ...
    <>
      <div className="fixed top-4 right-4 z-[50] flex flex-col-reverse items-center gap-3">
        ...
        >
          ...
          <span className="hidden md:inline font-rounded" style={{ fontFamily: "var(--font-cherry)" }}>Nhiệm vụ</span>
        ...
        <div ref={menuRef} className="relative">
          ...
          >
            {user ? (
              <img
                ...
                onError={(e) => {
                  (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${user.displayName || "U"}&background=FF7096&color=fff&rounded=true`;
                }}
              />
            ) : (
              <div className="w-8 h-8 flex items-center justify-center rounded-full bg-orange-50">
                <UserCircle className="w-5 h-5 text-amber-900" strokeWidth={2.5} />
              </div>
            )}
          </div>
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                {user && (
                  <div className="text-center pb-3 border-b-2 border-dashed border-zinc-100">
                    <p className="font-bold text-amber-900 text-sm">{user.displayName}</p>
                  </div>
                )}

                {/* CAPSULE TOGGLE: CHẾ ĐỘ HỌC */}
                <div className="flex flex-col gap-2">
                  <p className="text-xs font-bold text-amber-700 ml-2">Chế độ học:</p>
                  <div className="relative flex w-full bg-zinc-100 rounded-full p-1 border-2 border-zinc-200">
                    {/* Thanh trượt */}
                    <div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-full transition-transform duration-300 ease-in-out shadow-sm ${appMode === "fun" ? "translate-x-full bg-[#FF7096]" : "translate-x-0 bg-[#FFD166]"}`} />
                    
                    <button onClick={() => setAppMode("focus")} className={`flex-1 relative z-10 text-xs font-bold py-2 text-center transition-colors ${appMode === "focus" ? "text-amber-900" : "text-zinc-400 hover:text-zinc-600"}`}>👔 Tập trung</button>
                    <button onClick={() => setAppMode("fun")} className={`flex-1 relative z-10 text-xs font-bold py-2 text-center transition-colors ${appMode === "fun" ? "text-white" : "text-zinc-400 hover:text-zinc-600"}`}>🎈 Vui nhộn</button>
                  </div>
                </div>

                {/* LOGIN / LOGOUT BOTTOM */}
                {user ? (
                  <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-500 font-bold py-3 rounded-2xl transition-colors mt-1">
                    <LogOut className="w-4 h-4" /> Đăng xuất
                  </button>
                ) : (
                  <button onClick={handleLogin} className="w-full flex items-center justify-center gap-2 bg-[#06D6A0] hover:bg-[#05b889] text-white font-bold py-3 rounded-2xl transition-colors shadow-[0_4px_0_0_#048c68] active:translate-y-1 active:shadow-none border-b-4 border-[#048c68] mt-1">
      ...
      {/* MODAL NHIỆM VỤ */}
      <DailyQuestsModal isOpen={isQuestModalOpen} onClose={() => setIsQuestModalOpen(false)} />
    </>
  );
}