      ...
      >
        ...
        <div className="grid grid-cols-3 gap-1 bg-[#8C6D58]/10 p-1 rounded-2xl mx-4 mt-3 border border-[#8C6D58]/20">
          {(["shard", "exclusive", "consumable"] as const).map((tab) => (
            ...
            >
              {tab === "shard" ? "💎 MẢNH GHÉP" : tab === "exclusive" ? "👑 ĐỒ ĐỘC QUYỀN" : "🔮 BÙA CHÚ"}
            </button>
          ))}
        </div>
              ...
              {shardItems.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white border-2 hover:border-[#8C6D58] p-2 rounded-2xl flex flex-col justify-between h-[100px] cursor-pointer transition-all active:scale-95 shadow-xs relative ${selectedItem?.id === item.id ? "border-[#8C6D58] ring-2 ring-[#8C6D58]/20" : "border-zinc-200/60"
                      }`}
                  >
                    ...
                    <div className="absolute top-1 right-1 w-2.5 h-2.5 rounded-full" style={{ backgroundColor: rarityColor }} />
                  </div>
                );
              })}
            </div>
              ...
              {EXCLUSIVE_GOODS.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white border-2 hover:border-[#8C6D58] p-2 rounded-2xl flex flex-col justify-between h-[100px] cursor-pointer transition-all active:scale-95 shadow-xs relative ${owned ? "opacity-75" : ""
                      } ${selectedItem?.id === item.id ? "border-[#8C6D58] ring-2 ring-[#8C6D58]/20" : "border-zinc-200/60"
                      }`}
                  >
                    <div className="flex flex-col items-center">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-12 h-12 flex items-center justify-center bg-purple-50 rounded-2xl border border-purple-100 flex-shrink-0 overflow-hidden">
                        {/* Using placeholder emojis if svg files aren't created yet */}
                        <span className="text-2xl">{isDoubleBones ? "🏮" : "🔮"}</span>
                      </div>
                ...
                <div className="w-16 h-16 flex items-center justify-center bg-zinc-50 border-2 border-[#8C6D58]/10 rounded-2xl overflow-hidden flex-shrink-0">
                  {selectedType === "shard" ? (
                    ...
                    <span className="text-3xl">
                      {selectedItem.id === "buff_double_bones" ? "🏮" : selectedItem.id === "buff_lucky_gacha" ? "🔮" : "👑"}
                    </span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
