...
export function AuthButton({ isInline = false }: AuthButtonProps) {
  ...
  if (isInline) {
    return (
      <div ref={menuRef} className="relative z-30" style={{ fontFamily: "var(--font-cherry)" }}>
        <div
          onClick={() => setIsOpen(!isOpen)}
          className="flex items-center bg-white/90 backdrop-blur-md p-1 rounded-full border-2 border-[#FFE2D1] shadow-[0_4px_0_0_#FFE2D1] hover:-translate-y-1 active:translate-y-1 active:shadow-none transition-all cursor-pointer select-none"
        >
          {user ? (
            <img
              src={user.photoURL || ""}
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute top-full mt-3 right-0 bg-white border-4 border-[#FFE2D1] rounded-[2rem] p-4 shadow-xl w-64 origin-top-right flex flex-col gap-4 z-50"
            >
              {user && (
